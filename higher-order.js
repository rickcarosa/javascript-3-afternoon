...
let evenNumbers = mixedNumbers.filter(val => {
  return (val % 2 === 0);
});
...
//Code Here
let totalPopulation = populations.reduce((prev, val) => {
  return (prev + val)
});
...
let ordersTotal = orders.map(obj => {
  var salesTax = obj.price * obj.tax;
  return obj.price + salesTax;

});

// let ordersTotal = orders.reduce((prev, val) => {
//    return prev + val["price"] + val["tax"]
...
// let bobsTotal = purchases.map(obj => {
//   var sum = obj.owner["Bob"] + obj.price;
//   return obj.price + sum;
// })
let bobsTotal = purchases.filter(val => val.owner == 'Bob').reduce((prev, val) => prev + val.price, 0)
bobsTotal;
